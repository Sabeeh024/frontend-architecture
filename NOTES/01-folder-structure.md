...
| Feature-Sliced Design (supplementary — a scaled-up feature-based, see Q4) | `structure/fsd` |
...
**Who's allowed to import whom — and how do you enforce it?**
Policy: `shared` imports nothing domain-specific; features import `shared` and,
one-way, more-foundational features; the app/route layer imports anything;
nobody imports the app layer. Enforce with lint (`import/no-cycle`,
`import/no-restricted-paths`, `eslint-plugin-boundaries`) — deferred for now.

**When two features need the same data, does it get its own home?**
Escalation: (1) leave it if it's tiny; (2) let one feature own it and expose it
via its `index.js` (what `auth` does with the current user); (3) only when a
*second* feature genuinely needs it, promote it to a lower-level slice of its
own (`entities/`, `lib/`, a shared feature).

**Does `comments → auth` mean auth is really "infrastructure"?**
Yes — and that's the useful insight. "Feature" covers two different things: a
user-facing capability (`comments` — ship it, cut it) and cross-cutting plumbing
(`auth` — everything depends on it, it depends on nothing). Worth separating the
two even inside a flat `features/` folder.

**How does this hold up at 30 features?**
Feature-Sliced Design is the formalised answer: same two ideas (one-way imports,
split capability from plumbing) packaged as six fixed layers —
`app / pages / widgets / features / entities / shared` — with `ui/model/api`
segments in every slice and a linter enforcing "import only downward."
Branch `structure/fsd` has the whole Devlog app rebuilt that way as a reference.
Verdict: the packaging is worth it for large multi-team codebases; below that,
**feature-based folders + a couple of enforced rules + promoting shared code
only when a second consumer appears** gets you the same benefits without the
ceremony. FSD's re-gluing tax (splitting comment read/write into separate slices
forced the widget to re-wire them with `refetch()`) is the visible cost of
taking the rules further than the app needs.
